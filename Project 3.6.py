...
3.6 爲蛇增加了黑色邊框 (see english version)
"""
...
3.6 Added black outline to snakes; integrated console output into GUI window; removed debug output
"""
...
ladders = []
sresolution = 40
textbox1,textbox2,textbox3,textbox4 = [False]*4
...
def roll_dice(canvas, size, player_position):
	...
	"""
	global textbox1,textbox2,textbox3,textbox4
	if textbox1 != False:
		canvas.delete(textbox1)
		canvas.delete(textbox2)
		canvas.delete(textbox3)
	if textbox4 != False:
		canvas.delete(textbox4)
		textbox4 = False
	#print("Snakes: ")
	#for i in snakes:
		#print(f"{ i[0] + 1} -> {i[1] + 1}")
	...
	global nowPlayer
	textbox1 = canvas.create_text(size*40+60,size*40-20
	,text=f"It's {(nowPlayer+1)%players+1}'s turn, \nplease roll the dice.")
	#print(f"It's {nowPlayer}'s turn, please click enter to get dice toll")
	x = getRandomNumber()
	textbox2 = canvas.create_text(size*40+51,10,text=f"Dice number is {x}")
	#print(f"Dice number is {x}")
	u = playerLocation[nowPlayer]
	v = jump(u, x)
	#print(f"{nowPlayer} moves to {v + 1}")
	textbox3 = canvas.create_text(size*40+43,26,text=f"{nowPlayer+1} moves to {v + 1}")
	for i in snakes:
		if v == i[0]:
			#print(f"{nowPlayer} Encountered a snake and fell into {i[1] + 1}.")
			textbox4 = canvas.create_text(size*40+51,60,text=f"{nowPlayer+1} encountered a \nsnake and fell \ninto {i[1] + 1}.")
			v = i[1]

	for i in ladders:
		if v == i[0]:
			#print(f"{nowPlayer} Encountered a ladder and advanced into {i[1] + 1}!")
			textbox4 = canvas.create_text(size*40+55,60,text=f"{nowPlayer+1} encountered a \nladder and \nadvanced into {i[1] + 1}!")
			v = i[1]
...
def draw_snakes(canvas):
	...
	for item in snakes_coords: #A biezer curve is generated based on the randomly generated points and the point where the snake starts and ends. A snake is drawn on that biezer curve.
		...
		for i in range(csresolution-1):
			...
			canvas.create_oval(drawpointx+size+2,drawpointy+size+2,drawpointx-size-2,drawpointy-size-2,fill="black",outline="")
		for i in range(csresolution-1):
			lastlinep1x,lastlinep1y = getPointOnLine(templinelist[-1][0][i][0],templinelist[-1][1][i][0],templinelist[-1][0][i][1],templinelist[-1][1][i][1],csresolution,i)
			lastlinep2x,lastlinep2y = getPointOnLine(templinelist[-1][1][i][0],templinelist[-1][2][i][0],templinelist[-1][1][i][1],templinelist[-1][2][i][1],csresolution,i)
			drawpointx,drawpointy = getPointOnLine(lastlinep1x,lastlinep2x,lastlinep1y,lastlinep2y,csresolution,i)
...
def draw():
	...
	canvas = tk.Canvas(root, width=size * 40 +120, height=size * 40, bg='white')
	canvas.pack()
